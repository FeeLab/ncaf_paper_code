function ...
    [archive_burst_RS,archive_burst_length,archive_burst_start]=rank_surprise_burst(ISI,spikeMotif,limit,RSalpha)
...
begin_int = [];
end_int = [];
for i = unique(spikeMotif)
    isiValid = (ISI<limit) & (spikeMotif==i);

    ISI_limit=diff(isiValid);
    %first time stamp of these intervals
    begin_int=[begin_int find(ISI_limit==1)+1];
    %last time stamp of these intervals
    end_int=[end_int find(ISI_limit==-1)];
end
