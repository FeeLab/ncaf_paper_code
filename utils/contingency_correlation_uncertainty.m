...
function [tvals, frateCorr, confC] = contingency_correlation_uncertainty(unitSignal, np_fs, trigDelay, cW, unitI, unitNum, isNoise, plotAx, isNormed, ww, binW)
    ...
    iVals = ceil(tOrigin*np_fs/binW)+(-round(ww*np_fs/binW):round(ww*np_fs/binW));
    ...
    tvals = linspace(-(numel(frateCorr)-1)/2/np_fs*binW*1000, (numel(frateCorr)-1)/2/np_fs*binW*1000, numel(frateCorr));
